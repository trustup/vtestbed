...
if data[2]['name'] != None:
    ...
    print(rtu_os)
    for x in range(len(rtu)):
        gst = {'id': rtu[x], 'basevm_host': 'host_1', 'basevm_config_file': '{}/image_{}.xml'.format(path_img, rtu_os[x]),
               'basevm_type': 'kvm'}
        guest.append(gst)
...
fw_os = data[4]['os']
for x in firewall:
    gst = {'id': x, 'basevm_host': 'host_1', 'basevm_config_file': '{}/image_{}.xml'.format(path_img, fw_os),
           'basevm_type': 'kvm'}
    guest.append(gst)
...
with open(r'/home/ubuntu/PycharmProjects/temp/script_after_clone_restart.sh', 'w') as f:
    ...
    for y in total_number_vm2:
        ...
        #    f.write(' &>/dev/null &"\n')
        elif y != model[0] + '.eth0' and exit == 0:
            f.write("sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ")
            f.write("root@{} ".format(ip_list[y][0]))
            f.write('"route add default gw {} eth0;'.format(ip_list[y][1]))
            f.write('iptables -F;iptables -P INPUT ACCEPT; iptables -P OUTPUT ACCEPT"')
...
total_number_vm = [x + ".eth0" for x in total_number_vm]
with open(r'/home/ubuntu/PycharmProjects/temp/script_after_clone.sh', 'w') as f:
    ...
    for y in total_number_vm2:
        ...
        if y == model[0] + '.eth0' and exit == 0:
            ...
            f.write("su -c 'python3 /home/ubuntu/script_selenium.py' - ubuntu")
            f.write(' &>/dev/null &"\n')
        elif y != model[0] + '.eth0' and y != mtu[0] + '.eth0' and exit == 0:
            f.write("sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ")
            f.write("root@{} ".format(ip_list[y][0]))
            f.write('"iptables -F;iptables -P INPUT ACCEPT; iptables -P OUTPUT ACCEPT"')
            f.write("\n")
...
'''
with open(r'/home/ubuntu/PycharmProjects/temp/script_after_clone.sh','w') as g:
    y = model[0] + '.eth0'
    g.write("#!/bin/bash\n")
    ...
    g.write("sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ")
    g.write("root@{} ".format(ip_list[y][0]))
    g.write("su -c 'python3 /home/ubuntu/script_selenium.py' - ubuntu")
    g.write(' &>/dev/null &"\n')
    g.write("exit 0")
g.close()
'''
