...
if data['PLC']['name'] != None:
    ...
    for idx, x in enumerate(plc):
        gst = {'id': x, 'basevm_host': 'host_1', 'basevm_config_file': '{}/image_{}.xml'.format(path_img, plc_os[idx]),
               'basevm_type': 'kvm',
               'tasks': [{'copy_content': [
                   {'src': '{}/PLC/{}/mbconfig.cfg'.format(path_models, idx+1), 'dst': '/bin/cyberrange/OpenPLC_v3/webserver'},
                   {'src': '{}/PLC/{}/{}'.format(path_models, idx+1, plc_code[idx]), 'dst': '/bin/cyberrange/OpenPLC_v3'},
                   {'src': '{}/PycharmProjects/temp/script_selenium_plc{}.py'.format(path,idx+1), 'dst': dst_script_ubuntu},
               ]}]
              }
        guest.append(gst)
...
if data['GENERIC']['name'] != None:
    ...
    os = data['GENERIC']['os'].replace(" ","").split(",")
...
# FIREWALL
firewall = data['ROUTER-FIREWALL']['name'].replace(" ","").split(",")
fw_os = data['ROUTER-FIREWALL']['os'].replace(" ","").split(",")
...
for y in range(len(firewall)):
    if firewall[y] == entry_point:
        cl_g = {'guest_id': y, 'number': 1, 'forwarding_rules': data['ROUTER-FIREWALL']['forwarding'][y]['rules'],
                'entry_point': bool('yes')}
        clone_guest.append(cl_g)
    else:
        cl_g = {'guest_id': firewall[y], 'number': 1, 'forwarding_rules': data['ROUTER-FIREWALL']['forwarding'][y]['rules']}
        clone_guest.append(cl_g)
...
members = data['NETWORKS']['members'].replace(" ","")
...
target = []
fix = []
if data['SECURITY']['vulnerability'] != None:
    vuln = data['SECURITY']['vulnerability'].replace(" ","").split(";")
    target = data['SECURITY']['target'].replace(" ","").split(";")
...
if modeler == 'openmodelica':
    if model_os[0] == 'windows.7':
        with open(r'/home/ubuntu/PycharmProjects/temp/script.bat'.format(model[0]), 'w') as f:
            ...
            f.write('START /B /REALTIME "" C:\CyberRange\modelica\{}.exe -embeddedServer=opc-ua -rt=1 >null\n'.format(model[0]))
            if len(rtu) == 0:
...
if len(plc) != 0:
    for idx, x in enumerate(plc):
        login_user = 'openplc'
        login_pass = 'openplc'
        with open(r'/home/ubuntu/PycharmProjects/temp/script_selenium_plc{}.py'.format(idx+1), 'w') as f:
            f.write(
                "import selenium\nfrom selenium import webdriver\nfrom selenium.webdriver.common.keys import Keys\nimport json\nimport time\n")
            f.write("from selenium.webdriver.firefox.options import Options\n")
            f.write("options = Options()\noptions.headless = True\n")
...
###################################################################################################################################
# DATA JSON GENERATOR
ln = (len(sensors) + len(actuators))*2
ar = np.arange(0,ln,2)
...
if data['MTU']['set_alert']['type'] != None:
    tp = data['MTU']['set_alert']['type'].upper()
    type_alert.append(tp)
    for idx, y in enumerate(type_alert):
        if y == 'HIGH_LIMIT':
            vr = data['MTU']['set_alert']['variable'].replace(" ", "").split(">")
        ...
        if y == 'LOW_LIMIT':
            ...
            eventDetector.append({'xid':'PED_{}'.format(980000+idx),
                                  ...
                                  "alias":y})
        if y == 'CHANGE':
            vr = data['MTU']['set_alert']['variable'].replace(" ", "")
