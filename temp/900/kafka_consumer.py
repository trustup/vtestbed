...
for message in consumer:
    ...
    master = modbus_tcp.TcpMaster(host="900.1.1.2", port=502)
    try:
        if message[6]['900']['asset_name'] == 'aiv_robot':
            if message[6]['900']['actions'][0]['action_type'] == 'power_management':
                if message[6]['900']['actions'][0]['commands'][0]['action_name'] == 'off':
                    master.execute(1, cst.WRITE_MULTIPLE_REGISTERS, starting_address=10, output_value=[0.0], data_format='>f')

                if message[6]['900']['actions'][0]['commands'][0]['action_name'] == 'on':
    ...
    try:
        if message[6]['900']['asset_name'] == 'plc1':
            for z in range(4):
                try:
                    if message[6]['900']['actions'][z]['action_type'] == 'power_management':
                        if message[6]['900']['actions'][z]['commands'][0]['action_name'] == 'stop':
                            os.system('sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@900.1.1.5 "systemctl stop openplc.service"')
                        if message[6]['900']['actions'][z]['commands'][0]['action_name'] == 'start':
                            data = "systemctl start openplc.service; su -c 'python3 /home/ubuntu//script_selenium_plc1_restart.py' - ubuntu &>/dev/null &"
                            os.system('sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@900.1.1.5 "{}"'.format(data))
                    if message[6]['900']['actions'][z]['action_type'] == 'update_firmware':
                        if message[6]['900']['actions'][z]['commands'][0]['action_name'] == 'disable':
                            ...
                            os.system('sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@900.1.1.5 "{}"'.format(data))
                        if message[6]['900']['actions'][z]['commands'][0]['action_name'] == 'enable':
