    ...
    try:
        ...
        if exp != None:
            ...
            for idx, x in enumerate(source):
                filebeat_config.filebeat(x, source_os[idx], log_type[idx], ip_list['{}'.format(exp) + '.eth0'][0],
                                         port_kafka, port_beats, path_temp_script)
        ...
        after_clone.script_after_clone(sec_vuln, sec_target, ip_list, rtu, plc, model, model_os, mtu, hmi, hmi_os,
                                       firewall, generic, exp, read_topic, source, total_number_vm, dst_script_ubuntu, dst_script_windows, numb_cyber, path_temp_script)
...
with open(r'/home/ubuntu/virtualTestbed/config1_5_9.yaml') as f:
