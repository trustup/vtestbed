...
echo "  <memory>2048000</memory>" >> ${ABSPATH}images/${VM_ID}_config.xml;
...
#ADDED
echo "    <controller type='virtio-serial' index='0'>" >> ${ABSPATH}images/${VM_ID}_config.xml;
...
#ADDED
echo "    <channel type='spicevmc'>" >> ${ABSPATH}images/${VM_ID}_config.xml;
echo "      <target type='virtio' name='com.redhat.spice.0'/>" >> ${ABSPATH}images/${VM_ID}_config.xml;
echo "      <address type='virtio-serial' controller='0' bus='0' port='1'/>" >> ${ABSPATH}images/${VM_ID}_config.xml;
echo "    </channel>" >> ${ABSPATH}images/${VM_ID}_config.xml;
#/ADDED
...
echo "    <graphics type='spice' autoport='yes'>" >> ${ABSPATH}images/${VM_ID}_config.xml;
echo "      <image compression='off'/>" >> ${ABSPATH}images/${VM_ID}_config.xml;
echo "    </graphics>" >> ${ABSPATH}images/${VM_ID}_config.xml;
...
echo "      <model type='qxl' ram='65536' vram='65536' vgamem='16384' heads='1'/>" >> ${ABSPATH}images/${VM_ID}_config.xml;
echo "      <address type='pci' domain='0x0000' bus='0x00' slot='0x02' function='0x0'/>" >> ${ABSPATH}images/${VM_ID}_config.xml;
echo "    </video>" >> ${ABSPATH}images/${VM_ID}_config.xml;
echo "    <memballoon model='virtio'>" >> ${ABSPATH}images/${VM_ID}_config.xml;
echo "      <address type='pci' domain='0x0000' bus='0x00' slot='0x07' function='0x0'/>" >> ${ABSPATH}images/${VM_ID}_config.xml;
