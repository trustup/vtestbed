...
class ManageUsers(Modules):
    ...
    def add_account(self, new_account, new_passwd, full_name, os_type):
        ...
        if os_type=="windows.7" :
            ...
            command_string = "sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} 'bash -s' < {1}{5}/users_managing/add_user.sh {2} {3} yes {4}".format(self.addr, self.getAbsPath(), new_account, new_passwd, full_name_arg, INSTANTIATION_DIR)
        command = Command(command_string, desc)
        return command
    ...
    def modify_account(self, account, new_account, new_passwd, os_type):
        ...
        if os_type =="windows.7":
            ...
            command_string = "sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} 'bash -s' < {1}{5}/users_managing/modify_user.sh {2} {3} {4}".format(self.addr, self.getAbsPath(), account, new_account, new_passwd, INSTANTIATION_DIR)
        command = Command(command_string, desc)
        return command
