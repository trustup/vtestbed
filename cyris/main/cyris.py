    ...
    def main(self):
        ...
        for guest in self.guests:
            ...
            if guest.basevm_os_type in ('windows.7'):
                ...
                command = "sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} \"{1}\"\n".format(guest.getBasevmAddr(), add_gw_str)
                add_gw_str= "route add 0.0.0.0 mask 0.0.0.0 192.168.122.1"
                command += "sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} \"{1}\"\n".format(guest.getBasevmAddr(), add_gw_str)
            elif guest.basevm_os_type in ('centos.7'):
                command = "sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} route add default gw 192.168.122.1".format(guest.getBasevmAddr())
            if DEBUG:
                print command
            self.os_system(self.creation_log_file, command)
            ...
            for instance in host.getInstanceList():
                for clone_guest in instance.getCloneGuestList():
                    if clone_guest.getOsType().find("windows")!=-1:
                        ...
                        command = "sshpass -p theroot ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} tsdiscon 1".format(clone_guest.getNicAddrDict()["eth0"])
                        self.os_system(self.creation_log_file, command)
