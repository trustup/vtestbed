                    ...
                    for clone_guest in instance.getCloneGuestList():
                        if guest.getGuestId() == clone_guest.getGuestId():
                            ...
                            if guest.getBasevmOSType() =='windows.7':
                                command = "sshpass -p theroot scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {0}initif.conf root@{1}:'C:\CyberRange\initif'".format(self.directory, guest.getBasevmAddr())
                            else:
                                command = "sshpass -p theroot scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {0}initif.conf root@{1}:/bin/cyberrange/initif".format(self.directory, guest.getBasevmAddr())
